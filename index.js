		...
		prepare() {
			...
			return {
				Declaration(decl, { result }) {
					 ...
					 * @returns
					 */
					function parseValueForProperties({ type, value, nodes }) {
						...
						if (type === "function" && value === "var") {
							...
							// do some recursion to find the value...
							return parseValueForProperties(onlyWords?.[0]);
						}
						...
						const propDef = propMetadata.has(value)
							? propMetadata.get(value)
							: {
									decl: [decl],
									declUsing: [],
									propsUsing: [],
						...
						propMetadata.set(value, propDef);
					}
